...
class TestCreateOrder:
    ...
    def test_create_order_whith_auth(self, create_new_user):
        ...
        # Здесь добавить: with allure.step("Отправить POST-запрос на создание заказа с авторизацией")
        response = requests.post(
            URL.main_url + Endpoints.CREATE_ORDER,
            headers=headers,
            data=Ingredients.correct_ingredients_data
        )
        # Здесь добавить: with allure.step("Проверить, что заказ успешно создан")
    ...
    def test_create_order_without_auth(self):
        # Здесь добавить: with allure.step("Отправить POST-запрос на создание заказа без авторизации")
        response = requests.post(
            URL.main_url + Endpoints.CREATE_ORDER,
            data=Ingredients.correct_ingredients_data
        )
        # Здесь добавить: with allure.step("Проверить, что заказ успешно создан без авторизации")
        assert response.status_code == StatusCode.OK and response.json().get("success") is True
    ...
    def test_create_order_whithout_ingredients(self, create_new_user):
        token = create_new_user[1].json()["accessToken"]
        headers = {'Authorization': token}
        # Здесь добавить: with allure.step("Отправить POST-запрос на создание заказа без ингредиентов")
        response = requests.post(
            URL.main_url + Endpoints.CREATE_ORDER,
            headers=headers,
            data=Ingredients.incorrect_ingredients_data_without_filling
        )
        # Здесь добавить: with allure.step("Проверить, что заказ не был создан и вернулся статус 400")
    ...
    def test_create_order_incorrect_hash(self, create_new_user):
        token = create_new_user[1].json()["accessToken"]
        headers = {'Authorization': token}
        # Здесь добавить: with allure.step("Отправить POST-запрос на создание заказа с невалидным хэшем ингредиента")
        response = requests.post(
            URL.main_url + Endpoints.CREATE_ORDER,
            headers=headers,
            data=Ingredients.incorrect_ingredients_data_hash
        )
        # Здесь добавить: with allure.step("Проверить, что сервер вернул ошибку 500 и сообщение об ошибке присутствует")
